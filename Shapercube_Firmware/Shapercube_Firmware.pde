// Shapercube 2 Firmware 0.6
// Fork of Tonokip RepRap firmware rewrite based off of Hydra-mmm firmware.
// Licence: GPL
...
// dual head support, note: we use this also for bed. so tool 1, 2 = extruder, tool 3 = heatbed
byte tool_in_use = 0;
...
{ 
  ...
  if(E2_STEP_PIN > -1) pinMode(E2_STEP_PIN,OUTPUT);
  ...
  if(E2_DIR_PIN > -1) pinMode(E2_DIR_PIN,OUTPUT);
  ...
  if(E2_ENABLE_PIN > -1) if(!E2_ENABLE_ON) digitalWrite(E2_ENABLE_PIN,HIGH);
  ...
  if(E2_ENABLE_PIN > -1) pinMode(E2_ENABLE_PIN,OUTPUT);
...
inline void process_commands()
{
  ...
  if(code_seen('M'))
  {
    ...
    {
      case 104: // M104
        if (code_seen('S')) target_raw[tool_in_use] = temp2analog(code_value());
        break;
      case 105: // M105
        Serial.print("T:");
        //if (tool_in_use == 0){
        ...
        //}else{
          Serial.print(" T1:");
          Serial.print(analog2temp(analogRead(TEMP_1_PIN))); 
        //}
        
        Serial.print(" B:"); 
        Serial.println(analog2temp(analogRead(TEMP_2_PIN))); 
      ...
      case 109: // M109 - Wait for heater to reach target.
        ...
        while(current_raw < target_raw[tool_in_use]) {
      ...
      case 106: //M106 Fan On
        // Fixme: Shou
        analogWrite(FAN_PIN, FAN_0_VALUE);
        break;
...
void linear_move(unsigned long x_steps_remaining, unsigned long y_steps_remaining, unsigned long z_steps_remaining, unsigned long e_steps_remaining) // make linear move with preset speeds and destinations, see G0 and G1
{
  ...
  if (destination_e > current_e) 
  {
    if (tool_in_use == 0)
    {
      ...
      digitalWrite(E2_DIR_PIN,!INVERT_E2_DIR);    
    }
  }else{
    if (tool_in_use == 0)
    {
...
inline void disable_e2() { if(E2_ENABLE_PIN > -1) digitalWrite(E2_ENABLE_PIN,!E2_ENABLE_ON); }
...
inline void  enable_e2() { if(E2_ENABLE_PIN > -1) digitalWrite(E2_ENABLE_PIN, E2_ENABLE_ON); }

inline void manage_heater()
{
  manage_heater(TEMP_0_PIN, HEATER_0_PIN, HEATER_0_LOW_VALUE, HEATER_0_HIGH_VALUE, 0);
  if (HEATER_1_PIN > 0) manage_heater(TEMP_1_PIN, HEATER_1_PIN, HEATER_1_LOW_VALUE, HEATER_1_HIGH_VALUE, 1);
  if (HEATER_2_PIN > 0) manage_heater(TEMP_2_PIN, HEATER_2_PIN, HEATER_2_LOW_VALUE, HEATER_2_HIGH_VALUE, 2);
}

inline void manage_heater(byte temp_pin, byte heater_pin, byte low_val, byte high_val, byte tool)
{
  current_raw = analogRead(temp_pin);                  // If using thermistor, when the heater is colder than targer temp, we get a higher analog reading than target, 
  ...
  if(current_raw >= target_raw[tool])
   {
     analogWrite(heater_pin, low_val);
     digitalWrite(LED_PIN,LOW);
   }
  else 
  {
    analogWrite(heater_pin,high_val);
